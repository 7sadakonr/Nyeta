import { BlindMode, BoundingBox } from '@/types/assistant';
 ...
 */
export function extractGeminiText(data: GeminiResponseData | null | undefined): string {
...
export interface CallGeminiVisionParams {
    mode?: BlindMode;
    userPrompt?: string;
...
export async function callGeminiVision({
    ...
    temperature = 0.4,
    signal,
}: CallGeminiVisionParams = {}): Promise<string> {
    ...
    const data = (await response.json().catch(() => ({}))) as GeminiResponseData;
