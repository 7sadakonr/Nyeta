...
import { generateSessionToken, verifySessionToken, CALL_TOKEN_TTL_MS } from '@/server/auth/sessionAuth';
...
export async function POST(request: NextRequest) {
    try {
        ...
        let blindUserId: string | null = null;

        if (authHeader && authHeader.startsWith('Bearer ')) {
            const token = authHeader.slice(7).trim();
            const verification = verifySessionToken(token);
            if (verification && verification.role === 'blind') {
                blindUserId = verification.userId;
            }
        }
