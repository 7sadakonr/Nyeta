...
import { UseDataChannelResult } from './useDataChannel';

export type CaptureState = 'idle' | 'flash-on' | 'capturing' | 'sending' | string;

export interface UseCaptureHandlerProps {
    localStreamRef: RefObject<MediaStream | null>;
    localVideoRef: RefObject<HTMLVideoElement | null>;
    dataChannel: UseDataChannelResult | null;
}

export interface UseCaptureHandlerResult {
...
export function useCaptureHandler({
    ...
    localVideoRef,
    dataChannel
}: UseCaptureHandlerProps): UseCaptureHandlerResult {
    const [captureState, setCaptureState] = useState<string>('idle'); // 'idle' | 'flash-on' | 'capturing' | 'sending'
    const manualTorchRef = useRef<boolean>(false);
    ...
    const captureImage = useCallback(async (options?: { flash?: boolean }) => {
        ...
        try {
            ...
            if (options?.flash && !manualTorchRef.current) {
                if (videoTrack && typeof (videoTrack as any).applyConstraints === 'function') {
                    try {
                        ...
                        try {
                            await (videoTrack as any).applyConstraints({ torch: true });
                            turnedOnForCapture = true;
                        } catch (err2) {
            ...
            if (turnedOnForCapture && videoTrack) {
                try {
                    await (videoTrack as any).applyConstraints({ advanced: [{ torch: originalTorch }] });
                } catch (e) {
                    try {
                        await (videoTrack as any).applyConstraints({ torch: originalTorch });
                    } catch (err3) {
    ...
    useEffect(() => {
        ...
        const handleMessage = async (message: any) => {
            if (message.type === 'capture-request') {
                ...
                if (videoTrack && typeof (videoTrack as any).applyConstraints === 'function') {
                    try {
                        await (videoTrack as any).applyConstraints({ advanced: [{ torch: shouldFlash }] });
                    } catch (e) {
                        try {
                            await (videoTrack as any).applyConstraints({ torch: shouldFlash });
                        } catch (err2) {
