...
export interface GenerateSessionTokenParams {
    userId?: string | null;
    role: UserRole;
    callId?: string | null;
    ttlMs?: number;
 ...
 */
export function generateSessionToken({ userId, role, callId = null, ttlMs }: GenerateSessionTokenParams): string {
 ...
 */
export function verifySessionToken(token: string | null | undefined): DecodedSessionToken | null {
    ...
    try {
        const payload = JSON.parse(base64UrlDecode(encodedPayload)) as DecodedSessionToken;
...
export const ALLOWED_ROLE_EVENTS: Record<string, Record<string, UserRole[]>> = {
    'presence-volunteers': {
        'incoming-call': ['blind'],
 ...
 */
export function validateChannelPermission(tokenPayload: DecodedSessionToken | null, channelName: string): boolean {
 ...
 */
export function validateEventPermission(channelName: string, eventName: string): boolean {
 ...
 */
export function validateRoleEventPermission(
    tokenPayload: DecodedSessionToken | null,
    channelName: string,
    eventName: string,
