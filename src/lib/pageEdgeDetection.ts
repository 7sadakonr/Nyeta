 ...
 */
import { BoundingBox, Point2D, QuadCorners } from '@/types/assistant';
...
export interface PageAlignmentMetrics {
    detected: boolean;
    vw: number;
    vh: number;
    rotation: number;
...
export interface PageAnalysisResult {
    ...
    aligned: boolean;
    guidance: string;
    metrics: PageAlignmentMetrics | null;
}

function notDetectedResult(): PageAnalysisResult {
    return {
        detected: false,
        bounds: null,
        corners: null,
...
export async function preloadPageScanner(): Promise<void> {
    if (_scanner) return;
...
function quadArea(corners: QuadCorners): number {
...
function centroid(corners: QuadCorners): Point2D {
    const { tl, tr, br, bl } = corners;
    return {
...
function isValidQuadShape(corners: QuadCorners): boolean {
    const { tl, tr, br, bl } = corners;
...
function buildGuidance(metrics: PageAlignmentMetrics): { aligned: boolean; guidance: string } {
    if (!metrics.detected) {
...
function computeAlignmentFromCorners(corners: QuadCorners, vw: number, vh: number): PageAnalysisResult {
    ...
    const perspective: { tilt: string | null; roll: string | null } = { tilt: null, roll: null };
    if (widthRatio > 0.25) {
    ...
    const metrics: PageAlignmentMetrics = {
        detected: true,
        vw,
        vh,
        rotation,
...
function mapScanicCorners(scanCorners: any): QuadCorners | null {
